...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
         ...
         */
        public Path(Path copyPath, Edge extendBy) { // TODO
     ...
     */
    protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException { // TODO

        // start and end does not exist
        if (!containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException("Specified start/end data item does not exist.");
        }

        if (start == null || end == null) {
            throw new NoSuchElementException("Specified start/end data item is null.");
        }

        // start and end are the same vertex
        if (vertices.get(start).equals(vertices.get(end))) {
            return new Path(vertices.get(start));
        }

        // keeps track of vertices to which the shortest path has been found
        LinkedList<Vertex> visited = new LinkedList<>();
        PriorityQueue<Path> queue = new PriorityQueue<>();

        queue.add(new Path(vertices.get(start))); // add starting vertex to queue
        visited.add(vertices.get(start));

        // finds the shortest path from specified start and end vertices
        while (!queue.isEmpty()) {

            // lowest cost path in the queue
            Path curr = queue.remove();

            if (curr.end.equals(vertices.get(end))) { // shortest path is found
                return curr;
            }

            for (Edge edge : curr.end.edgesLeaving) { // add any unvisited edges to path
                if (!visited.contains(edge.target)) {
                    queue.add(new Path(curr, edge));
                }
            }

            // since this path is currently the shortest path in the queue, add vertex to visited
            visited.add(curr.end);
        }

        throw new NoSuchElementException("No such path exist.");
    }
